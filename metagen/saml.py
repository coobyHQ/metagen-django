...
class MetagenSaml2LogoutResponse(OneLogin_Saml2_Logout_Response):
    """
    ...
    def __init__(self, settings, response=None, method='redirect'):
        ...
        """
        if method == 'redirect':
            super(MetagenSaml2LogoutResponse, self).__init__(settings, response)
        elif method == 'post':
...
class MetagenSaml2Auth(OneLogin_Saml2_Auth):
    ...
    def process_slo(self, keep_local_session=False, request_id=None, delete_session_cb=None):
        ...
        if post_data:
            ...
            logout_response = MetagenSaml2LogoutResponse(self.__settings, get_data['SAMLResponse'], method)
